...
#define FORKRUN_LOADABLES(X) \
    ...
    X(ring_numa_ingest,     ring_numa_ingest_main,    "ring_numa_ingest <infd> <outfd> <nodes> [ordered]", "Run NUMA topological ingest") \
    X(ring_indexer_numa,    ring_indexer_numa_main,   "ring_indexer_numa <memfd> <node_id>", "Run NUMA chunk indexer") \
    X(ring_numa_scanner,    ring_numa_scanner_main,   "ring_numa_scanner <memfd> <node_id> <spawn_fd> <nodes>", "Run NUMA localized scanner") \
...
static SHELL_VAR *bind_var_or_array(const char *name, char *value, int flags) {
    if (!name) return NULL;
    char *lb = strchr(name, '[');
    if (!lb || name[strlen(name) - 1] != ']') return bind_variable(name, value, flags);
    size_t base_len = (size_t)(lb - name);
    char *base_tmp = (char *) xmalloc(base_len + 1);
    memcpy(base_tmp, name, base_len); base_tmp[base_len] = '\0';
    size_t idx_len = strlen(lb + 1) - 1;
    char *idx_tmp = (char *) xmalloc(idx_len + 1);
    memcpy(idx_tmp, lb + 1, idx_len); idx_tmp[idx_len] = '\0';
    char *base_s = savestring(base_tmp); char *idx_s  = savestring(idx_tmp); char *val_s  = savestring(value);
    xfree(base_tmp); xfree(idx_tmp);
    SHELL_VAR *var = find_variable(base_s);
    if (!var) { var = make_new_array_variable(base_s); if (!var) { xfree(base_s); xfree(idx_s); xfree(val_s); return NULL; } }
    SHELL_VAR *ret = NULL;
...
static int xcreate_anon_file(const char *name) {
    ...
    if (use_memfd) {
        ...
        if (fd >= 0) return fd;
        if (errno == EINVAL) { fd = syscall(__NR_memfd_create, name, 0); if (fd >= 0) return fd; }
    }
    ...
    fd = mkstemp(path);
    if (fd < 0) { snprintf(path, sizeof(path), "/tmp/forkrun.XXXXXX"); fd = mkstemp(path); }
    if (fd >= 0) unlink(path);
    return fd;
}
...
static int ring_numa_ingest_main(int argc, char **argv) {
    ...
    while (1) {
        ...
        while (current_major > 0 && current_major - atomic_load_acquire(&g_state->ingest_publish_idx) >= META_RING_SIZE - 4) {
            cpu_relax();
        }
        ...
        if ((uint64_t)n == chunk_size) {
            ...
            if (consecutive_full >= 4 && chunk_size < max_chunk) {
                chunk_size *= 2; consecutive_full = 0;
            }
        } else {
            chunk_size = 4 * 1024 * 1024ULL; consecutive_full = 0;
        }
    }
    ...
    uint64_t v = 1; SYS_CHK(write(evfd_ingest_eof, &v, 8));
    return EXECUTION_SUCCESS;
}
...
// ==============================================================================
#define ADAPTIVE_FLOW_CONTROL(state_ptr, is_stalled, _node_id_arg) do { \
    (void)(is_stalled); \
    batch_counter++; \
    batches_since_calc++; \
    ...
    while (1) {
        ...
        if (my_tail >= my_head) {
            ...
            for (int i = 0; i < num_nodes; i++) {
                ...
                if (h > t && (h - t) > (uint64_t)max_bl) { max_bl = (int)(h - t); steal_target = i; }
            }
            if (max_bl == 0) {
                if (atomic_load_acquire(&g_state->ingest_eof_idx) != ~(uint64_t)0) goto scanner_eof;
            }
        ...
        while (!((act_end_flag = atomic_load_acquire(&meta->actual_end)) & FLAG_META_READY)) { cpu_relax(); }
        ...
        if (current_major > 0) {
            ...
            while (!((prev_act_end = atomic_load_acquire(&prev_meta->actual_end)) & FLAG_META_READY)) { cpu_relax(); }
            actual_start = prev_act_end & ~FLAG_META_READY;
        }

        if (actual_start >= actual_end) {
            NUMA_SCANNER_FLUSH(0, 0, true, meta->major_id, 0, actual_start, actual_start);
            NUMA_ADAPTIVE_COMMIT(true);
            continue;
        }
...

        {
            ...
            if (current_stall) stall_meter  = (stall_meter  + _xLim) >> 1;
            else               stall_meter  >>= 1;
            if (atomic_load_relaxed(&local_state->active_waiters) > 0) starve_meter = (starve_meter + _xLim) >> 1;
            else starve_meter >>= 1;
        }
        ...
        while (current_p_offset < chunk_end) {
            ...
            if (byte_mode) {
                ...
                if (limit_items > 0) {
                    ...
                    else if (prev + take > limit_items) { take = limit_items - prev; limit_reached = true; }
                }
                ...
                if (pending_lines >= L || current_p_offset >= chunk_end || limit_reached) flush = true;
                else if (stall_meter >= (W + DAMPING_OFFSET - 3) && starve_meter >= (W + DAMPING_OFFSET - 3)) {
            ...
            if (flush) {
                ...
                NUMA_SCANNER_FLUSH(pending_lines, stride, is_last, meta->major_id, minor_idx, batch_start, current_p_offset);
                minor_idx++;
...
static int ring_scanner_main(int argc, char **argv) {
    ...
    while (status != 1 || p < end) {
        {
            uint64_t _xLim = W + DAMPING_OFFSET;
            if (atomic_load_relaxed(&state[0].active_waiters) > 0) starve_meter = (starve_meter + _xLim) >> 1;
            else starve_meter >>= 1;
        }
        ...
        if ((p >= end || force_refill) && status != 1) {
            ...
            if (n > 0 && (uint64_t)n > prev_avail) {
                ...
                if (!atomic_load_acquire(&state[0].ingest_complete)) {
                    struct pollfd pfds[2] = { { .fd = evfd_ingest_data, .events = POLLIN }, { .fd = evfd_ingest_eof, .events = POLLIN } };
                    if (poll(pfds, 2, 0) > 0) { if (pfds[1].revents & POLLIN) atomic_store_release(&state[0].ingest_complete, 1); }
                }
        ...
        if (byte_mode) {
            ...
            if (avail >= L) { take = L; flush = true; first_wait_ts = 0; }
            else if (avail > 0) {
                if (status == 1) { take = avail; flush = true; }
            ...
            if (flush) {
                ...
                p += take; batch_start += take; total_scanned++; pending_lines = 0;
            ...
            pending_lines += found; total_scanned += found;

            if (limit_items > 0 && total_scanned >= limit_items) status = 1;

            if (pending_lines > 0) {
                uint64_t xLim = W + DAMPING_OFFSET;
                if (pending_lines >= L || status == 1 || limit_reached) flush = true;
                ...
                if (flush) {
                    ...
                    batch_start = current_p_offset; pending_lines = 0; first_wait_ts = 0;
                } else force_refill = true;
            } else force_refill = true;
        }
            ...
            if (!unordered_mode) {
                while (heap_sz > 0) {
                    ...
                    if (memfd_mode) {
                        ...
                        if (use_zerocopy) robust_sendfile(1, top.pkt.fd, &offset, top.pkt.len);
                        else ring_copy_chunk(top.pkt.fd, 1, offset, top.pkt.len);
