// forkrun_ring.c v9.4 (Golden Master - Physics Fixed)
// ======================================================================================
...
static int ring_init_main(int argc, char **argv) {
    ...
    // Only trigger Byte Mode if start-bytes is set or lines are explicitly disabled
    if (cfg_l.state_a == CONF_DISABLE || cfg_b.state_a == CONF_SET) {
        byte_mode = true;
    }
    ...
    if (byte_mode) {
        ...
        state->cfg_line_max    = cfg_b.b; // Use bytes config as max limit in line mode
    }
