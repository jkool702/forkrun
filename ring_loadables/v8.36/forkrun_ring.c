// forkrun_ring.c v8.36
// ======================================================================================
// CHANGES vs v8.35:
// 1. CONFIG: Moved configuration to SharedState.
// 2. INIT: 'ring_init' now accepts flags: --bytes, --limit, --timeout, --greedy, --out.
// 3. SCANNER: Implemented 3 Byte-Mode behaviors (Strict, Greedy, Timed).
// 4. SCANNER: Implemented Global Line Limit.
// 5. SCANNER: 'offset[N+1]' fencepost logic preserved.
// 6. PIPES: Unbuffered 'write' fix from v8.35 preserved.
// ======================================================================================
...
#define FORKRUN_LOADABLES(X) \
    X(ring_init,            ring_init_main,           "ring_init [--bytes=N] [--limit=N] [--timeout=N] [--greedy] [--out=VAR]", "Initialize ring with config") \
...
struct SharedState {
    ...
    // CONFIGURATION (Set by ring_init)
    uint64_t cfg_limit;       // Max lines/chunks. 0 = Infinite.
    uint64_t cfg_byte_chunk;  // Byte mode chunk size. 0 = Line Mode.
    uint64_t cfg_timeout_us;  // Byte Mode Timeout. 0=Greedy, MAX=Strict.
...
static int ring_init_main(int argc, char **argv) {
    ...
    state->cfg_byte_chunk = 0;  // Line Mode
    state->cfg_timeout_us = 0;  // Default (Greedy for Lines, Strict for Bytes, overridden below)
    ...
    for (int i = 1; i < argc; i++) {
        if (strncmp(argv[i], "--limit=", 8) == 0) {
            ...
            state->cfg_byte_chunk = (uint64_t)atoll(argv[i] + 8);
            // Default to Strict (Infinite Timeout) for Byte Mode
            state->cfg_timeout_us = UINT64_MAX; 
        } else if (strncmp(argv[i], "--timeout=", 10) == 0) {
            state->cfg_timeout_us = (uint64_t)atoll(argv[i] + 10);
        } else if (strcmp(argv[i], "--greedy") == 0) {
            state->cfg_timeout_us = 0;
        } else if (strncmp(argv[i], "--out=", 6) == 0) {
            out_array_name = argv[i] + 6;
        } else if (argv[i][0] != '-') {
            // Legacy positional argument for out_array
            out_array_name = argv[i];
        }
    }

    // Set signed_batch_size to 1 if byte mode (disable aggregation)
    if (state->cfg_byte_chunk > 0) {
...
static int ring_scanner_main(int argc, char **argv) {
    ...
    // Read Config from Shared State
    uint64_t global_limit = state->cfg_limit;
    uint64_t chunk_bytes  = state->cfg_byte_chunk;
    uint64_t timeout_us   = state->cfg_timeout_us;
    
    // Tuning Defaults (Line Mode only)
    ...
    if (chunk_bytes == 0) { // Only parse line tuning if in Line Mode
        if ((tmp_var = get_string_value("nLinesMax"))) {
            long val = strtol(tmp_var, NULL, 10);
            if (val > 0) Lmax = (uint64_t)((val > 65535) ? 65535 : val);
        }
        if ((tmp_var = get_string_value("nBytesMax"))) {
            if (strcmp(tmp_var, "L2") == 0) BytesMax = val_l2;
                ...
                long long val = strtoll(tmp_var, NULL, 10);
                if (val > 0) BytesMax = (uint64_t)val;
            }
        }
    }
    ...
    uint64_t first_wait_ts = 0; // For timeout mode
    ...
    while (status != 1 || p < end) {
        ...
        if ((p >= end || force_refill) && status != 1) {
            ...
            if (n > 0) {
                ...
                if (status != 1) {
                    ...
                    if (chunk_bytes > 0 && starving && timeout_us < UINT64_MAX) {
                        // In Byte+Timed mode, if waiting, we might need to wake up earlier
                        // to flush partial.
                        if (first_wait_ts == 0) first_wait_ts = get_us_time();
                        uint64_t now = get_us_time();
                        if (now - first_wait_ts >= timeout_us) {
                            poll_timeout = 0; // Timeout expired, fall through to flush
                        } else {
                            // Wait for remainder
        ...
        // 2. Scan Logic (Branch: Byte Mode vs Line Mode)
        ...
        if (chunk_bytes > 0) {
            ...
            if (global_limit > 0 && total_scanned >= global_limit) {
                status = 1; // Limit hit, stop.
                break;
            }
            ...
            if (avail >= chunk_bytes) {
                ...
                // Partial available. Check sub-modes.
                if (status == 1) { // EOF
                    take = avail;
                    flush = true;
                    ...
                    if (timeout_us == 0) { // Greedy
                        take = avail;
                        flush = true;
                    } else if (timeout_us < UINT64_MAX) { // Timed
                        if (first_wait_ts > 0 && (get_us_time() - first_wait_ts >= timeout_us)) {
                            take = avail;
                            flush = true;
                            first_wait_ts = 0;
                        }
            ...
            if (flush) {
                ...
                // Commit
                uint64_t pk = (uint64_t)batch_start;
                if (take < chunk_bytes) pk |= FLAG_PARTIAL_BATCH;
                
                state->offset_ring[local_scan_idx & RING_MASK] = pk;
                state->stride_ring[local_scan_idx & RING_MASK] = 1; // 1 chunk
                ...
                if (backlog > W && W < W_max_val) {
                     uint64_t needed = (backlog > W_max_val) ? W_max_val - W : backlog - W;
                     if (needed > 0) {
                         char sbuf[64];
            ...
            if (global_limit > 0) {
                uint64_t rem = global_limit - total_scanned;
    ...
    // --- Tail Logic (Only relevant for Line Mode really, but harmless for Byte) ---
    // In Byte mode, we don't really have a "Tail" rewind because we don't overshoot 
    // unless we change logic. But let's keep it safe.
    
    // ... [Copy existing tail logic, but wrap in check] ...
    if (chunk_bytes == 0) {
...
// ... [Remainder of file (claim, workers, pipe, splice) is unchanged from v8.35] ...
// ... [Just copy/paste the functions from v8.35 for ring_claim, ring_worker, etc.] ...

// (The remaining functions are identical to v8.35, I won't re-paste them to save space unless requested)
