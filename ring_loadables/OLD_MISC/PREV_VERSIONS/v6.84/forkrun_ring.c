...
// Fixes: Multi-Arch Support (AArch64, RISC-V, PPC64, S390x) for cpu_relax()

#ifndef _GNU_SOURCE
#define _GNU_SOURCE 1
#endif

// Ensure 64-bit file offsets on 32-bit architectures (ARMv7)
...
// ... (Rest of the file is identical to v6.83) ...
                    ...
                    while (offset < st.st_size) {
                        ...
                        if (sent < 0) {
                            ...
                            if (errno == EINVAL || errno == ENOSYS || errno == ENOTSOCK || errno == EBADF) {
                                // Fallback for TTY/Pipe if sendfile fails
                                char buf[32768];
                                ssize_t n;
