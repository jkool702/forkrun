...
// Optimization: Aggressive Tail-Shrinking (Reverted Damping), Smart Auto-Tuning
                ...
                if (phase == 0) {
                    ...
                    if (now_us - last_calc_us > 5000) {
                        ...
                        if (l_target > L) {
                            ...
                            atomic_store_release(&state->batch_change_idx, local_write_idx);
                            atomic_store_release(&state->signed_batch_size, -(int64_t)L);
                        } else if (l_target < L && wait_meter >= 30) {
                            // Aggressive Tail-Shrinking
                            L = (L + l_target) / 2;
                            if (L < 1) L = 1;
                            atomic_store_release(&state->batch_change_idx, local_write_idx);
                            atomic_store_release(&state->signed_batch_size, -(int64_t)L);
