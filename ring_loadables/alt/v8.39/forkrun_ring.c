// forkrun_ring.c v8.39
// ======================================================================================
// CHANGES vs v8.38:
// 1. ORDER: Replaced 'dprintf' in 'ring_order' with 'snprintf'+'write'.
//    This guarantees that the batch index header is flushed to the FD *before*
//    'sendfile' begins writing data, preventing potential out-of-order output.
// ======================================================================================
...
#define FORKRUN_LOADABLES(X) \
    ...
    X(ring_order,           ring_order_main,          "ring_order <FD> <PFX|memfd> [idx]","Reorder output") \
    ...
    while (status != 1 || p < end) {
        ...
        if (byte_mode) {
            ...
            if (flush) {
                ...
                // Update Workers (Backlog logic for Byte Mode)
                uint64_t r_idx = atomic_load_relaxed(&state->read_idx);
                uint64_t backlog = local_scan_idx - r_idx;
                if (backlog > W && W < W_max) {
                     uint64_t needed = (backlog > W_max) ? W_max - W : backlog - W;
                     if (needed > 0) {
                         char sbuf[64];
                         int slen = snprintf(sbuf, sizeof(sbuf), "%lu\n", needed);
                         if (slen > 0) SYS_CHK(write(fd_spawn, sbuf, slen));
        ...
        while(1) {
            ...
            if (pfd.revents & (POLLIN | POLLHUP | POLLERR)) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (op->idx == next_idx) {
                        // PRINT HEADER IF -K MODE (Safely using snprintf+write)
                        if (index_mode) {
                            char hbuf[64];
                            int hlen = snprintf(hbuf, sizeof(hbuf), "\n\n\x1c%" PRIu64 "\x1d\n", op->idx);
                            if (hlen > 0) write(1, hbuf, hlen);
                        }
