...
// ======================================================================================
// CHANGES vs v8.37:
// 1. ORDER: Updated 'ring_order' to accept optional 'index' argument.
//    If present, it prints the batch index header ("\n\n\x1C%lu\x1D\n") to stdout
//    before releasing the batch data. This supports the -K flag.
// ======================================================================================
...
static int ring_init_main(int argc, char **argv) {
    ...
    if (state != NULL) {
        ...
        // NOTE: Config is NOT reset on soft-reset. It persists unless destroy is called.
...
static int ring_order_main(int argc, char **argv) {
    ...
    bool index_mode = (argc > 3); // Crude check, but covers [index] opt
        ...
        while(1) {
            ...
            if (pfd.revents & (POLLIN | POLLHUP | POLLERR)) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (op->idx == next_idx) {
                        // PRINT HEADER IF -K MODE
                        if (index_mode) {
                            // Format: \n\n \x1c INDEX \x1d \n
                            // Note: We flush stdout to ensure header appears before sendfile data
                            dprintf(1, "\n\n\x1c%" PRIu64 "\x1d\n", op->idx);
                        }
