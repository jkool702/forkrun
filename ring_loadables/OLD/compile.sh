...
gcc forkrun_ring_loadables.c $CFLAGS $INCLUDES -march=x86-64-v2 -o forkrun_ring_v2.so
strip --strip-all forkrun_ring_v2.so
# 2. Build v3 (Standard / AVX2)
echo "Building v3 (AVX2)..."
gcc forkrun_ring_loadables.c $CFLAGS $INCLUDES -march=x86-64-v3 -o forkrun_ring_v3.so
strip --strip-all forkrun_ring_v3.so
# 3. Build v4 (Extreme / AVX-512)
echo "Building v4 (AVX-512)..."
gcc forkrun_ring_loadables.c $CFLAGS $INCLUDES -march=x86-64-v4 -o forkrun_ring_v4.so
strip --strip-all forkrun_ring_v4.so
echo "Building native (AVX-512)..."
gcc forkrun_ring_loadables.c $CFLAGS $INCLUDES -march=native -o forkrun_ring.so
