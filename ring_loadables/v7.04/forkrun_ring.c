...
// Combination: v7.03 (Lookahead Fix) + v6.116 (Tail Logic)
...
static int ring_scanner_main(int argc, char **argv) {
            ...
            if (pending_lines >= L || status == 1 || limit_reached || starvation) {
                ...
                if (phase == 0) {
                    ...
                    if (status == 2) phase = 2; 
                        ...
                        // FIX: Use local_scan_idx (v7.03 fix logic)
    ...
    // (Replacing the v7.03 "flush everything" logic with v6.116 logic)
    ...
    // In v7.03, local_write_idx is the committed edge. local_scan_idx is the lookahead.
    if (local_scan_idx > local_write_idx) {
    ...
    if (local_scan_idx > local_write_idx) {
        // Rewind to the start of the first uncommitted batch.
        tail_start_offset = state->offset_ring[local_write_idx & RING_MASK] & ~FLAG_PARTIAL_BATCH;
    } else {
        // No uncommitted batches. Tail starts at current pending.
        tail_start_offset = batch_start;
    }
    
    // 4. Reset Scanner Internal State for Tail Ring
    // We leave local_write_idx (Main Ring) alone! It's frozen.
    ...
    // 5. Compute R (Reduction Factor)
    uint64_t R = 1;
    if (L > 1) R = 1 + (7 * fast_log2(L) / 16);
    if (R < 1) R = 1;
...
restart_loop:
    ...
    if (!using_tail_ring) {
        ...
        // CHECK FOR OVERSHOOT (Clamp & Restart logic from v6.115)
        if (my_read_idx + claim_count > w_curr) {
             ...
             if (atomic_load_acquire(&state->tail_active)) {
                 ...
                 // If we claimed 'ghost' slots (0 valid), we must restart
                 // to see the 'tail_active' check at the top of the loop.
    ...
    // Synthetic Monotonic ID: MainRingEnd + TailIndex
    uint64_t export_idx = using_tail_ring ? (atomic_load_relaxed(&state->write_idx) + my_read_idx) : my_read_idx;
