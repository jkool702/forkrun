// forkrun_ring.c v9.8 (Golden Master - Complete)
// ======================================================================================
...
#define FORKRUN_LOADABLES(X) \
    ...
    X(ring_version,         ring_version_main,        "ring_version [-a|-t|-m|-g]",     "Print version/build info") \
...
static int ring_init_main(int argc, char **argv) {
    ...
    if (out_array_name) {
        ...
        int created_cnt = 0;
        int failure = 0;
        for (uint64_t i = 0; i < state->cfg_w_max; i++) {
...
static int ring_fallow_main(int argc, char **argv) {
    ...
    // Reverted to simple blocking read loop (No poll)
    ssize_t n_read;
    while ((n_read = read(fd_in, ops, sizeof(ops))) > 0) {
...
static int ring_version_main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "-t") == 0) show_time = 1;
        else if (strcmp(argv[i], "-m") == 0) show_arch = 1;
        else if (strcmp(argv[i], "-g") == 0) show_gcc = 1;
    }

    if (show_all) {
        printf("forkrun_ring v%s\n", FORKRUN_RING_VERSION);
    ...
    if (show_time) printf("%s %s\n", __DATE__, __TIME__);
    if (show_arch) printf("%s\n", RING_ARCH);
    if (show_gcc)  printf("%s\n", __VERSION__);
