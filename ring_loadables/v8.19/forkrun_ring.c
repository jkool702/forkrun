...
restart_loop:
    while (1) {
        ...
        struct pollfd pfds[3] = { { .fd = evfd_data, .events = POLLIN }, { .fd = evfd_eof,  .events = POLLIN }, { .fd = fd_escrow[0], .events = POLLIN } };
        while(1) {
    ...
    if (claim_count == 1) {
        ...
        if (!in_tail && sbatch_now > 0 && !(state->offset_ring[my_read_idx & RING_MASK] & FLAG_PARTIAL_BATCH)) final_lines = sbatch_now;
