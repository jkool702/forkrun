...
#            F=(f3)
            ...
            for Fk in "${F[@]}"; do
                ...
                #for GCk in {,-k,-u}\ {,-l\ 1:-1}\ {/usr/bin/true,/usr/bin/echo,/usr/bin/printf\ '%s\n'}$'\n' -U\ {,-l\ 1:1024}\ {':',echo,printf\ '%s\n'}$'\n' {-s,-b\ 524288,-b4096\ -s}\ {:,/usr/bin/cat,/usr/bin/tee}$'\n'; do
                    ...
                    ((K++))
                    echo "($K): time { frun $GCk <$Fk >/dev/null; }" > ./.tmp.out
                    { time { frun --stats $GCk <$Fk >/dev/null 2>>./.tmp.out; echo >> ./.tmp.out; }; } 2>&1 | sed -zE 's/^.*real/real/' | tee ./.time >> ./.tmp.out
                    echo >> ./.tmp.out
                    getCPU >> ./.tmp.out
                    cat ./.tmp.out
                    sleep 1
                    ...
                    ((K++))
                    echo "($K): time { frun $GCk <$Fk | wc -l; }" > ./.tmp.out
                    { time { frun --stats $GCk <$Fk 2>>./.tmp.out | wc -l >> ./.tmp.out; } 1>&$fd1; } 2>&1 | sed -zE 's/^.*real/real/' | tee ./.time >> ./.tmp.out
                    echo >> ./.tmp.out
                    getCPU >> ./.tmp.out
                    cat ./.tmp.out
                    sleep 1
                    ...
                    ((K++))
                    echo "($K): time { cat $Fk | frun $GCk >/dev/null; }" > ./.tmp.out
                    { time { cat $Fk | frun --stats $GCk >/dev/null 2>>./.tmp.out; echo >> ./.tmp.out; }; } 2>&1 | sed -zE 's/^.*real/real/' | tee ./.time >> ./.tmp.out
                    echo >> ./.tmp.out
                    getCPU >> ./.tmp.out
                    cat ./.tmp.out
                    sleep 1
                    ...
                    ((K++))
                    echo "($K): time { cat $Fk | frun $GCk | wc -l; }" > ./.tmp.out
                    { time { cat $Fk | frun --stats $GCk 2>>./.tmp.out | wc -l >> ./.tmp.out; } 1>&$fd1; } 2>&1 | sed -zE 's/^.*real/real/' | tee ./.time >> ./.tmp.out
                    echo >> ./.tmp.out
                    getCPU >> ./.tmp.out
                    cat ./.tmp.out
                    sleep 1
