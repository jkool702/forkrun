...
# TL;DR SUMMARY 
# 11 tests were run on `parallel -m`, `xargs -P $(nproc) -d $'\n'`, and `forkrun`. AFAIK, these are the fastest parallelization methods supported by `parallel` and `xargs`, respectively.
...
# ---> on average (looking at total time between all tests), forkrun was around 1.7x as fast `xargs -P $(nproc) -d $'\n'` and around 7.3x as fast as `parallel -m`
# ---> in all tests forkrun was between 1.42x - 2.73x faster than `xargs -P $(nproc) -d $'\n'` and between 3.64x - 15.39x faster than `parallel -m`
#
# NOTE: though not shown here, when restricted to processing 1 line at a time (`parallel` ; `xargs -P $(nproc) -d $'\n' -L 1` ; `forkrun -l 1`), the performance gap is even larger:
#       forkrun tends to be at least 3-4x as fast as `xargs -P $(nproc) -d $'\n' -L 1` and >10x as fast as `parallel`
...
forkrun:        100%            100%            100%            100%            100%            100%            100%            100%            100%            100%            100%            100%           
...
declare -F forkrun 1>/dev/null 2>&1 || { [[ -f ./forkrun.bash ]] && source ./forkrun.bash; } || source <(curl https://raw.githubusercontent.com/jkool702/forkrun/main/forkrun.bash)
...
for nfun in "${tests[@]}"; do
	...
	printf 'Testing forkrun...' >&2
	mapfile -t C < <({ 
		printf '%s\n' '-----forkrun------'; 
		time { find "${findDir}" -type f | forkrun ${nfun} 2>/dev/null | wc -l; }; 
	} 2>&1)
	printf '....done\n' >&2
	sleep 1
	...
	${testParallelFlag} && { printf 'parallel: \t%s%% \t(%s%% %s than forkrun)\n' "$tdA" "$tddA" "$vA"; }
	printf 'xargs:    \t%s%% \t(%s%% %s than forkrun)\n' "$tdB" "$tddB" "$vB"
	printf 'forkrun:  \t100%%\n' 
...
Performance counter stats for '/bin/bash -c shopt -s extglob; source /mnt/ramdisk/forkrun.bash; for nfun in sha1sum sha256sum sha512sum sha224sum sha384sum md5sum  "sum -s" "sum -r" cksum b2sum "cksum -a sm3"; do find /mnt/ramdisk/usr -type f | forkrun ${nfun} | wc -l;  done' (5 runs):
...
Testing xargs.........done
Testing forkrun.......done


RESULTS FOR sha1sum
---------------------

-----parallel-----      ------xargs-------      -----forkrun------
...
parallel:       941%    (841% slower than forkrun)
xargs:          172%    (72% slower than forkrun)
forkrun:        100%
...
parallel:       590%    (490% slower than forkrun)
xargs:          168%    (68% slower than forkrun)
forkrun:        100%
...
parallel:       714%    (614% slower than forkrun)
xargs:          142%    (42% slower than forkrun)
forkrun:        100%
...
parallel:       576%    (476% slower than forkrun)
xargs:          161%    (61% slower than forkrun)
forkrun:        100%
...
parallel:       751%    (651% slower than forkrun)
xargs:          152%    (52% slower than forkrun)
forkrun:        100%
...
parallel:       887%    (787% slower than forkrun)
xargs:          176%    (76% slower than forkrun)
forkrun:        100%
...
parallel:       1539%   (1439% slower than forkrun)
xargs:          216%    (116% slower than forkrun)
forkrun:        100%
...
parallel:       914%    (814% slower than forkrun)
xargs:          181%    (81% slower than forkrun)
forkrun:        100%
...
parallel:       1660%   (1560% slower than forkrun)
xargs:          273%    (173% slower than forkrun)
forkrun:        100%
...
parallel:       802%    (702% slower than forkrun)
xargs:          153%    (53% slower than forkrun)
forkrun:        100%
...
parallel:       364%    (264% slower than forkrun)
xargs:          172%    (72% slower than forkrun)
forkrun:        100%
