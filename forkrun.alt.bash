    ...
    ${pipeReadFlag} || {
        ...
        if ${nullDelimiterFlag}; then
            echo """
                ...
                { dd if=\"/proc/self/fd/${fd_read0}\" bs=1 count=1 status=none skip=\$((\${fd_read_pos##*$'\t'}-\${fd_read_pos0##*$'\t'}-1)) | read -t 1 -r -d ''; } || {"""
        else
...
p_PID+=(\${p{<#>}_PID})""" )"
    ...
    ) {fd_continue}<><(:) {fd_inotify}<><(:) {fd_nAuto}<><(:) {fd_nOrder}<><(:) {fd_nOrder0}<><(:) {fd_read}<"${fPath}" {fd_read0}<"${fPath}" {fd_write}>"${fPath}" {fd_stdin}<&0 {fd_stdout}>&1 {fd_stderr}>&2
