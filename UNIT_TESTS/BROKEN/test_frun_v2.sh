#!/usr/bin/env bash
# ============================================================================
# FORKRUN COMPREHENSIVE TEST SUITE (FIXED)
# ============================================================================
# Tests all major modes, flags, and edge cases of forkrun.
# Self-verifying with clear PASS/FAIL indicators.
# ============================================================================
...
run_test() {
  ...
  elif [[ "$stdout_content" != "$expected_output" ]]; then
    passed=false
...
# Test that output matches expected order (for -k ordered mode)
# Usage: run_test_ordered <test_name> <command> <expected_output> [expected_exit=0]
run_test_ordered() {
  local test_name="$1"
  local cmd="$2"
  local expected_output="$3"
  ...
  if [[ "$exit_code" -ne "$expected_exit" ]]; then
    ...
    # For ordered mode, compare line by line preserving order
    local exp_line actual_line line_num=0
    while IFS= read -r exp_line && IFS= read -r actual_line <&3; do
      ((line_num++))
      if [[ "$exp_line" != "$actual_line" ]]; then
        passed=false
        fail_reason="line $line_num: expected '$exp_line', got '$actual_line'"
        break
      fi
    done 3< <(echo "$stdout_content") <<< "$expected_output"

    # Check if one file has more lines than the other
    if $passed; then
      if IFS= read -r dummy <&3; then
        passed=false
        fail_reason="actual output has more lines than expected"
      elif IFS= read -r dummy; then
        passed=false
        fail_reason="expected output has more lines than actual"
      fi
    fi
  fi
  ...
  if $passed; then
    ...
    echo -e "    ${YELLOW}Command:${NC} $cmd"
    echo -e "    ${YELLOW}Expected (first 10 lines):${NC}"
    echo "$expected_output" | head -10 | sed 's/^/        /'
    echo -e "    ${YELLOW}Actual (first 10 lines):${NC}"
    echo "$stdout_content" | head -10 | sed 's/^/        /'
    [[ -n "$stderr_content" ]] && {
      echo -e "    ${YELLOW}Stderr:${NC}"
      echo "      $(echo "$stderr_content" | sed 's/^/        /')"
    }
...
# Generate test data - NOTE: These are the actual strings that will be output
# forkrun passes these as arguments to printf "%s\n", so they come out exactly as-is
echo -e "line1\nline2\nline3\nline4\nline5\nline6\nline7\nline8\nline9\nline10" > "$LINE_INPUT"
...
# Unordered mode can output in any order, so we check that all lines are present
run_test "Default mode" \
  "cat '$LINE_INPUT' | frun printf \"%s\\n\"" \
  "$(cat "$LINE_INPUT" | sort -V)" \
  0

# Test 2: Ordered mode (-k)
# Ordered mode MUST preserve input order exactly
run_test_ordered "Ordered mode (-k)" \
  "cat '$LINE_INPUT' | frun -k printf \"%s\\n\"" \
  "$(cat "$LINE_INPUT")" \
  0

# Test 3: Realtime mode (-u)
# Realtime is also unordered
...
run_test_ordered "Buffered ordered (--buffered -k)" \
...
# -L forces exact count per batch, but output order may vary due to parallel execution
...
run_test_ordered "Record limit (-n 5)" \
  "cat '$LINE_INPUT' | frun -k -n 5 printf \"%s\\n\"" \
  "$(head -n5 "$LINE_INPUT")" \
  0

# Test 19: Limit with unordered (still should only output 5 lines, any order)
run_test "Limit with unordered (-n 5)" \
  "cat '$LINE_INPUT' | frun -n 5 printf \"%s\\n\"" \
  "$(head -n5 "$LINE_INPUT" | sort -V)" \
  0

# Test 20: Timeout (--timeout 100000) - just test that it's accepted
...
run_test_ordered "Insert mode (-i)" \
...
run_test_ordered "Insert with custom command (-i)" \
...
run_test_ordered "Safe quoting with spaces" \
  "cat '$SPACE_INPUT' | frun printf \"%s\\n\"" \
  "$(cat "$SPACE_INPUT")" \
  0

# Test 26: Unsafe mode (-U) - word splitting occurs
...
run_test_ordered "Explicit safe mode (+U)" \
  "cat '$SPACE_INPUT' | frun +U printf \"%s\\n\"" \
  "$(cat "$SPACE_INPUT")" \
  0
...
run_test_ordered "Stdin + ordered (-s -k)" \
  "cat '$LINE_INPUT' | frun -s -k cat" \
  "$(cat "$LINE_INPUT")" \
  0

# Test 29: Byte mode + realtime (unordered)
...
run_test_ordered "Byte + ordered (-b 50 -k)" \
...
# Expected: warning on stderr, normal output (unordered)
...
# Test 36: Dry run (-N)
# Dry run just prints commands, not output
run_test "Dry run (-N)" \
...
# Test 40: Stats flag (--stats) - only prints in NUMA mode
...
# Test 45: Tab characters
CTRL_INPUT="$TEST_DIR/ctrl.txt"
printf 'a\tb\nc\td\n' > "$CTRL_INPUT"
run_test_ordered "Tab characters" \
...
run_test_ordered "Combination: -s -k -n 5" \
...
run_test_ordered "Combination: --nodes=2 -j 4 -l 2 -k" \
...
run_test_ordered "Alias: --keep-order" \
