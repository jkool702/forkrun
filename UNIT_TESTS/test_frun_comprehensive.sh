...
run_test_regex L "L16a: Worker SIGKILL: full teardown, pipeline terminates" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && crash_kill9() { kill -9 \$BASHPID; }; seq 1 10 | FORKRUN_EXTRA_FUNCS=\"crash_kill9\" frun -l 1 crash_kill9 2>/dev/null; true'" \
    ".*" 0 false
...
run_test_regex L "L16b: Worker SIGKILL after partial output: no corruption" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && crash_kill9_partial() { echo \"BEFORE_SIGKILL_\$1\"; kill -9 \$BASHPID; }; seq 1 10 | FORKRUN_EXTRA_FUNCS=\"crash_kill9_partial\" frun -l 1 crash_kill9_partial 2>/dev/null | grep -c BEFORE_SIGKILL || true'" \
    "^0$" 0 false
...
run_test_regex L "L16c: Worker SIGKILL in -k mode: pipeline terminates" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && crash_kill9_k() { kill -9 \$BASHPID; }; seq 1 10 | FORKRUN_EXTRA_FUNCS=\"crash_kill9_k\" frun -k -l 1 crash_kill9_k 2>/dev/null; true'" \
    ".*" 0 false
...
run_test_regex L "L16d: Worker SIGKILL in -s mode: no deadlock" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && crash_kill9_s() { kill -9 \$BASHPID; }; seq 1 10 | FORKRUN_EXTRA_FUNCS=\"crash_kill9_s\" frun -s -l 1 crash_kill9_s 2>/dev/null; true'" \
    ".*" 0 false
...
run_test_regex L "L16e: Worker SIGKILL with -j 4: surviving workers shut down" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && crash_kill9_j4() { if [[ \"\$1\" == \"5\" ]]; then kill -9 \$BASHPID; else echo \"\$1\"; fi; }; seq 1 20 | FORKRUN_EXTRA_FUNCS=\"crash_kill9_j4\" frun -j 4 -l 1 crash_kill9_j4 2>/dev/null; true'" \
    ".*" 0 false
...
run_test_regex L "L17a: Scanner EOF (input closes early): pipeline terminates" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && (seq 1 5; exit 0) | frun -l 1 echo'" \
    ".*" 0 false
...
run_test_regex L "L17b: Scanner failure (input dies mid-stream): pipeline terminates" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && (seq 1 3; exit 1) | frun -l 1 echo 2>/dev/null; true'" \
    ".*" 0 false
...
run_test_regex L "L17c: Scanner failure with -k: pipeline terminates (no hang)" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && (seq 1 3; exit 1) | frun -k -l 1 echo 2>/dev/null; true'" \
    ".*" 0 false
...
run_test_regex L "L17d: Scanner failure mid-processing: no corrupted output" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && (seq 1 3; exit 1) | frun -k -l 1 echo 2>/dev/null | grep -v \"^[0-9]*$\" | wc -l || true'" \
    "^0$|^1$" 0 false
...
run_test_regex L "L17e: Scanner failure in -s mode: no deadlock" \
    "timeout 10 bash -c 'source \"$FRUN_SOURCE\" && (seq 1 3; exit 1) | frun -s -l 1 cat 2>/dev/null; true'" \
    ".*" 0 false
