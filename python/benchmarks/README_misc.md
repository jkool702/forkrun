# Archived ad-hoc scripts (formerly misc_python_benchmarks)
...
python3 python/benchmarks/throughput/ab.py v1 200000
```
...
- Most `throughput/` + `streaming/` scripts read `/tmp/bm.txt`,
  ...
  (see `python/benchmarks/ml/ml_data_gen.py`).
- Tokenize scripts need `/tmp/tok.jsonl` + `/tmp/tok.jsonl.vocab`
  (see `python/benchmarks/tokenize/tokenize_data_gen.py`) and set
